...
Face::Send (Ptr<Packet> packet)
{
  ...
  FwCsTag fwCsTag;
  bool tagExists2 = packet->RemovePacketTag(fwCsTag);
  if(tagExists2) {
    if(fwCsTag.Get()==1) {
      fwCsTag.Decrement();
    }  
    packet->AddPacketTag(fwCsTag);
  }

 ////////////////////////////////ADD///////////////////////////////////////////////
/*在接口处改变内容存储标记量，即将contentCopyTag值减1
*/
  FwcontentCopyTag contentCopyTag;
  bool tagExists3 = packet->RemovePacketTag (contentCopyTag);
  if (tagExists3)
    {
